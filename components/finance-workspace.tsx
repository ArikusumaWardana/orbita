...
import { Banknote, Bot, CalendarDays, Check, CheckSquare2, ChevronDown, ChevronLeft, ChevronRight, Home, ListFilter, Loader2, Moon, Plus, Tags, Sun, Trash2, TrendingDown, TrendingUp, WalletCards, X } from "lucide-react";
...
export function FinanceWorkspace({ initialPockets, initialCategories, initialTransactions, userName }: { initialPockets: Pocket[]; initialCategories: Category[]; initialTransactions: LedgerTransaction[]; userName: string }) {
  ...
  const paginatedGroups = useMemo(() => groups.slice((page - 1) * GROUPS_PER_PAGE, page * GROUPS_PER_PAGE), [groups, page]);
  ...
  return <div className="app-shell">
    ...
    <main className="workspace"><header className="topbar"><div><p className="eyebrow">Arus kas pribadi</p><h1>Keuangan</h1></div><div className="top-actions"><NotificationCenter /><button className="icon-button mobile-theme" onClick={toggleTheme} aria-label="Ganti tema">{theme === "dark" ? <Sun /> : <Moon />}</button><button className="secondary-button finance-pocket-button" onClick={() => setDialog("pocket")}><Plus /> Dompet</button><button className="primary-button" onClick={() => setDialog("transaction")}><Plus /></button></div></header>
      ...
      <section className="task-panel finance-panel"><div className="panel-heading"><div><p className="section-kicker">Catatan transaksi</p><h2>Riwayat harian</h2></div><button type="button" className="secondary-button compact-button" onClick={() => setDialog("category")}><Plus /> Kategori</button></div><div className="finance-filters"><SelectField label="Dompet" value={pocketFilter} onValueChange={handlePocketChange} options={[{ value: "all", label: "Semua dompet", icon: <WalletCards /> }, ...pockets.map((pocket) => ({ value: pocket.id, label: pocket.name, icon: <WalletCards /> }))]} /><SelectField label="Jenis" value={typeFilter} onValueChange={handleTypeChange} options={[{ value: "all", label: "Semua jenis", icon: <ListFilter /> }, { value: "income", label: "Pemasukan", icon: <TrendingUp /> }, { value: "expense", label: "Pengeluaran", icon: <TrendingDown /> }]} /><SelectField label="Kategori" value={categoryFilter} onValueChange={handleCategoryChange} options={[{ value: "all", label: "Semua kategori", icon: <Tags /> }, ...categories.filter((item) => typeFilter === "all" || item.type === typeFilter).map((item) => ({ value: item.id, label: item.name, icon: <Tags /> }))]} /><label className="date-filter-field">Dari<input type="date" value={dateFrom} max={dateTo} onChange={(event) => handleDateFromChange(event.target.value)} /></label><label className="date-filter-field">Sampai<input type="date" value={dateTo} min={dateFrom} onChange={(event) => handleDateToChange(event.target.value)} /></label>{(pocketFilter !== "all" || typeFilter !== "all" || categoryFilter !== "all" || dateFrom !== defaultDateFrom || dateTo !== defaultDateTo) && <button type="button" className="clear-finance-filters" onClick={handleResetFilters}>Reset filter</button>}</div>
        <WeeklyTrend transactions={visible} endDate={dateTo} />
        {groups.length === 0 ? <div className="state-card"><span className="empty-orbit"><WalletCards /></span><h3>Belum ada transaksi</h3><p>Catat pemasukan atau pengeluaran pertama untuk mulai menghitung saldo dompet.</p><button className="secondary-button" onClick={() => setDialog("transaction")}><Plus /> Catat transaksi</button></div> : <>
          <div className="transaction-groups">{paginatedGroups.map(([date, items]) => { const incoming = items.filter((item) => item.type === "income").reduce((sum, item) => sum + item.amount, 0); const outgoing = items.filter((item) => item.type === "expense").reduce((sum, item) => sum + item.amount, 0); return <section key={date}><header><strong>{new Intl.DateTimeFormat("id-ID", { weekday: "long", day: "numeric", month: "long", year: "numeric", timeZone: "UTC" }).format(new Date(`${date}T00:00:00Z`))}</strong><span>Masuk {money(incoming)} · Keluar {money(outgoing)}</span></header>{items.map((item) => <article className="transaction-row" key={item.id}><span className={`transaction-icon ${item.type}`}>{item.type === "income" ? <TrendingUp /> : <TrendingDown />}</span><div><strong>{categoryName(item.categoryId)}</strong><p>{item.description || pocketName(item.pocketId)}</p><small>{pocketName(item.pocketId)}</small></div><strong className={item.type}>{item.type === "income" ? "+" : "-"}{money(item.amount)}</strong><button className="icon-button delete-button" onClick={() => remove(item.id)} aria-label="Hapus transaksi"><Trash2 /></button></article>)}</section>; })}</div>
          {totalPages > 1 && (
            <div className="finance-pagination">
              <span>
                Halaman {page} dari {totalPages} ({visible.length} transaksi)
              </span>
              <div className="finance-pagination-controls">
                <button
                  type="button"
                  className="finance-pagination-button"
                  disabled={page <= 1}
    ...
    <nav className="bottom-nav"><Link href="/today"><Home /><span>Hari ini</span></Link><Link href="/"><CheckSquare2 /><span>Task</span></Link><Link href="/events"><CalendarDays /><span>Agenda</span></Link><Link href="/finance" className="active"><WalletCards /><span>Keuangan</span></Link><Link href="/assistant"><Bot /><span>Asisten</span></Link></nav>
