...
type EventInput = Pick<EventItem, "title" | "description" | "location" | "supportLink" | "eventAt" | "eventEndAt">;
...
function EventCard({ event, past, edit, remove, addReminder, editReminder, removeReminder }: { event: EventItem; past: boolean; edit: () => void; remove: () => void; addReminder: () => void; editReminder: (reminder: EventReminder) => void; removeReminder: (reminder: EventReminder) => void }) {
  return <motion.article className="event-card" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, x: -12 }}>
    <div className="event-card-main"><div className="event-date"><CalendarDays aria-hidden="true" /><span>{formatEventTime(event)}</span></div><h3>{event.title}</h3>{event.description && <p>{event.description}</p>}{event.location && <div className="event-location"><MapPin aria-hidden="true" />{event.location}</div>}{event.supportLink && <a className="event-support-link" href={event.supportLink} target="_blank" rel="noopener noreferrer"><ExternalLink aria-hidden="true" /><span>Buka link pendukung</span></a>}</div>
...
function EventDialog({ event, close, submit }: { event: EventItem | null; close: () => void; submit: (input: EventInput) => Promise<void> }) {
  ...
  async function onSubmit(formEvent: FormEvent<HTMLFormElement>) { formEvent.preventDefault(); setSaving(true); setError(""); const data = new FormData(formEvent.currentTarget); try { const endValue = hasRange ? String(data.get("eventEndAt")) : ""; await submit({ title: String(data.get("title")), description: String(data.get("description")), location: String(data.get("location")), supportLink: String(data.get("supportLink")), eventAt: new Date(String(data.get("eventAt"))).toISOString(), eventEndAt: endValue ? new Date(endValue).toISOString() : null }); } catch (caught) { setError(caught instanceof Error ? caught.message : "Agenda belum dapat disimpan."); setSaving(false); } }
