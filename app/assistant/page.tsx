...
import { dailyLimitState } from "@/lib/ai-limit";
...
export default async function AssistantPage() {
  ...
  const limit = dailyLimitState(profile.data.ai_daily_request_count, profile.data.ai_request_reset_at, new Date(), String(profile.data.timezone || "Asia/Makassar"));
  return <AssistantWorkspace initialMessages={(history.data as AssistantMessageRow[]).reverse().map(assistantMessageFromRow)} initialRemaining={limit.remaining} pockets={(pockets.data as PocketRow[]).map(pocketFromRow)} categories={categories.data as CategoryRow[]} userName={session?.user?.name ?? "Pengguna"} />;
}
